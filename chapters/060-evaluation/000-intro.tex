...
The primary issue is that due to time and resource constraints, thorough load testing was not possible in the realm of this thesis. Therefore, the scalability of the operator could not be validated and may not achieve sufficient throughput for large fleets.
...
The decision was made to simplify the debugging process, since Ansible produces verbose logs, which would have to be parsed and forwarded in an equal manner to the user by the operator, resulting in more obfuscation and possible failures.
...
An important note here is a limitation of Ansible itself, which is that deleting something from a playbook will not automatically remove the configuration from the machine.

...
It is commonly used for login into remote systems for management.
...
While \gls{ssh} does support the central signing and distribution of host keys, it is more common to have each host generate its own key and for clients to maintain a central database of what key is associated with what host.
In \gls{ssh} a host may have several host keys, each one for a different cryptographic algorithm.
...
Before login, the public key of the key pair must be placed in onto the \gls{ssh} server, so it can be read by the \gls{ssh} server implementation.
A user may then use it's private key to authenticate against the server.
A user may have several key pairs, and a server may have multiple keys registered for a user.
In this authentication protocol, the critical knowledge, the priviate key, never leaves the client, but is only validated via a challenge-response check.
\parencite{sshauthrfc, ssharchitecturerfc}

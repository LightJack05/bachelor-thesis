...
Assuming a \gls{deployment} object specifies that three instances of a \gls{pod} with an \gls{nginx} container should be running, the controller responsible for \gls{deployment} objects will reconcile the object and assess its condition to ensure that there are actually three instances running.
\parencite{poulton2025kubernetes, promise-theory, k8scontrollers, k8sclustercomponents}
...
It is possible to implement such checks using admission webhooks, but validating logical consistency via admission webhooks is generally discouraged \parencite{k8sadmissionwebhooksgoodpractices}.
Validation of the actual logical validity and the ability to move the observed state to the desired state should be determined at reconcile time.
...
Every condition then has additional properties, the primary one indicating whether the condition is active, with additional ones indicating a reason as to why the resource is in the specific condition, a human-readable message from the controller as well as the transition time.

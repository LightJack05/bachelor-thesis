Ansible instead uses scripts called \texttt{Playbooks} to describe the target state of the machine. These \texttt{Playbooks} are usually kept idempotent to ensure that re-running them does not alter the state of the target machine if it already is in the requested state.

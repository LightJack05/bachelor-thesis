Ansible instead uses scripts called \texttt{Playbooks} to describe the target state of the machine, which are usually held idempotent to ensure that re-running the same \texttt{Playbook} doesn't alter the state of the target machine if it already is in the requested state.
...
\texttt{Roles} are reusable units of \texttt{Tasks} which can be used to split configurations into reusable chunks to, for example, apply a similar configuration repeatedly to several logicial targets without having to contain them in \texttt{Playbooks}.
